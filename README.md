...
*이 게임은 특이하게도, 사용자는 아이디어만 제공할 뿐 모든 기술적인 역할은 챗 지피티 코덱스가 맡습니다. 사용자가 바쁘고 여유가 없을 경우, 코덱스가 사용자의 역할까지 모두 도맡을 수 있습니다. 관련 내용은 'codex's room'을 꼭 참고하세요. 그 방의 지침을 읽고 그것을 최우선으로 지켜나가면 됩니다.
...
`config/debug.js`에서 `DEBUG` 값을 `true`로 설정하면 모듈의 로그가 출력됩니다.
배포용 빌드에서는 `false`로 두어 콘솔 출력을 끌 수 있습니다.
