...
어떤 유닛이 어떤 스킬을 몇개 가져오느냐에 따라 차이가 발생.

## 유닛의 종류
- 플레이어 : 모든 스킬들 중 두 가지를 고를 수 있다.
...
- 챔피언 몬스터 : 용병의 형상을 하고 몬스터 특성, 디버프, 몬스터 스킬을 가져온다.
- 성기사 : 고유 용병. 버프 스킬 한 개와 용병 스킬 한 개를 가져온다.
- 다크나이트 : 고유 몬스터. 디버프 스킬 한 개와 몬스터 스킬 한 개를 가져온다.
- 보스 몬스터 : 층 5개 마다 배치. 버프 스킬 한 개와 몬스터 스킬 한 개를 가져온다.(아군으로 섭렵 불가능)
...
어떤 유닛들은 각각의 능력치 옆에 별의 숫자가 0~3개 붙는다. 별의 갯수가 많을 수록 레벨당 능력치 증가량이 높다.
유닛은 한 번에 9개 이상의 별을 가질 순 없다.

## 타일

유닛은 타일을 장비할 수 있다. 장비할 경우 해당 타일의 항상 유닛 도트 이미지 뒤에 붙어서 따라다닌다.
타일마다 특별한 능력치를 제공한다.
플레이어는 맵에 랜덤으로 생성되는 타일을 밟아서 모을 수 있다.
타일은 같은 종류 3개를 모아서 상위 타일로 만들 수 있다.
총 티어5까지의 타일을 종류당 만들 수 있다.
타일 인벤토리가 따로 있다.
